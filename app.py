...
def download_image_from_drive(file_name):
    ...
    if not all_items or not search_target:
        return None
    target_id = None
...
def upload_and_overwrite(target_filename, content_bytes):
    ...
    try:
        ...
        media = MediaIoBaseUpload(
            BytesIO(content_bytes), mimetype='image/jpeg', resumable=True)
...
def generate_pdf_original_style(df, center_name):
    ...
    for i, row in df.iterrows():
        ...
        table_data.append([Paragraph(str(i+1), thai_styles["CellStyle"]), Paragraph(d_thai, thai_styles["CellStyle"]), Paragraph(row['name'], thai_styles["CellStyle"]), Paragraph(fmt_time(row['time_in']),
                          thai_styles["CellStyle"]), Paragraph(fmt_time(row['time_out']), thai_styles["CellStyle"]), Paragraph(row['status'], thai_styles["CellStyle"]), Paragraph("", thai_styles["CellStyle"])])
    tbl = Table(table_data, colWidths=[
                35, 100, 130, 60, 60, 80, 70], repeatRows=1)
    tbl.setStyle(TableStyle([("GRID", (0, 0), (-1, -1), 0.5, colors.black), ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey), ("VALIGN", (0, 0),
                 (-1, -1), "MIDDLE"), ("ALIGN", (0, 0), (-1, -1), "CENTER"), ("TOPPADDING", (0, 0), (-1, -1), 3.5), ("BOTTOMPADDING", (0, 0), (-1, -1), 3.5)]))
    story.append(tbl)
                ...
                try:
                    with Image.open(img_stream) as PIL_img:
                        ...
                        PIL_img.convert('RGB').save(
                            temp_io, format="JPEG", quality=90)
                        temp_io.seek(0)
                        im = RLImage(temp_io)
...
for idx in df_idx:
    ...
    with st.expander(f"📅 {row['date']} - {row['name']}"):
        ...
        for i, col in enumerate(["img_in1", "img_out1"]):
            ...
            if img_d:
                c_img[i].image(
                    img_d, caption=f"Drive: {target_filename}", use_container_width=True)
            else:
            ...
            if new_f is not None:
                if target_filename in ["", "0", "nan"]:
                    ...
                    with st.spinner(f"กำลังอัปโหลด..."):
                        # ทำการอัปโหลด
                        upload_and_overwrite(
                            target_filename, new_f.getbuffer())
...
if st.button("🖨️ ออกรายงาน PDF", use_container_width=True, type="primary"):
    with st.spinner("กำลังสร้าง PDF..."):
        pdf = generate_pdf_original_style(
            st.session_state.main_df.loc[df_idx], sel_center)
        st.download_button(
            "📥 ดาวน์โหลด PDF", pdf, f"{sel_center}.pdf", "application/pdf", use_container_width=True)
