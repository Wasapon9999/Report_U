...
if sel_center == "--- กรุณาเลือกศูนย์ที่ต้องการตรวจ ---":
    ...
    with main_container.container():
        ...
        with cent_co:
            ...
            try:
                st.image("logo.J", width=125)
            except:
            st.markdown("<h3 style='text-align: center; color: #1d71b8;'> สู้ๆ ครับ </h3>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        
        # ส่วนข้อความต้อนรับ
        st.markdown("<h1 style='text-align: center; font-size: 2em;'>ระบบจัดการรายงาน USO1</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; color: #1d71b8; font-size: 1.1em;'><b> เวลาปฏิบัติงาน เดือนมีนาคม 2026 </b></p>", unsafe_allow_html=True)
        st.divider()
