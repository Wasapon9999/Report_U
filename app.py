...
# --- 3. Utility & PDF Generator (Original Style) ---
...
def generate_pdf_original_style(df, center_name):
    ...
    sig_left = [Paragraph("....................................", thai_styles["Signature"]), Spacer(1, 6), Paragraph(f"( {emp_name} )", thai_styles["Signature"]), Paragraph("ผดล.ประจำศูนย์", thai_styles["Signature"])]
    sig_right = [Paragraph("....................................", thai_styles["Signature"]), Spacer(1, 6), Paragraph("( ...................................... )", thai_styles["Signature"]), Paragraph("ตำแหน่ง_______________________", thai_styles["Signature"])]
...
def render_main_ui(center):
    with main_container.container():
        ...
        # ปุ่มสร้าง PDF กลับมาอยู่ด้านบนเพื่อความสะดวก
        if st.button("🖨️ ออกรายงาน PDF (รูปแบบเดิม)", use_container_width=True, type="primary"):
        ...
        for idx in df_idx:
            ...
            # ✅ ปรับเป็น expanded=False เพื่อไม่ให้กางออกอัตโนมัติ (ประหยัด RAM)
            with st.expander(f"📅 {row['date']} - {row['name']}", expanded=False):
...
if sel_center == "--- กรุณาเลือกศูนย์ที่ต้องการตรวจ ---":
    with main_container.container():
        ...
        st.divider()
        st.info("💡 เลือกศูนย์ทางด้านซ้ายมือ แล้วคลิกที่วันที่เพื่อตรวจสอบรายละเอียดครับ")
